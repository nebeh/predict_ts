...
class BaseLine:
    ...
    def start_points(self):
        ...
        threshold = 0.7 # вероятность того, что следующий ценовой уровень будет равен предыдущему
    ...
    def stock_mask_trend(self):
        # симуляция стока
        new_odrer = self.order
        likelihood = np.zeros_like(new_odrer)
        # начальная точка = начальное значение товаров на складе
        stock_start = int(self.lam[0]*300) + 1
        stock = np.random.randint(stock_start)
        for i in range(new_odrer.size):
            if stock <= 0:
                likelihood[i] = 1
                if np.random.rand() < 0.05: # в среднем, 20 дней без товара
                    stock = np.random.randint(stock_start)
                new_odrer[i] = 0
            elif self.order[i]>stock:
                likelihood[i] = stock/self.order[i]
                new_odrer[i] = stock
                stock -= self.order[i]
            else:
                stock -= self.order[i]
                likelihood[i] == 0
        if np.sum(likelihood) > likelihood.size/2:
            new_odrer = self.order
            likelihood = np.zeros_like(new_odrer)

        return likelihood, new_odrer
    ...
    #TODO: Использовать более приближенные к реальности кривые сезонностей и трендов
...
# Класс Competitor - симулятор для конкурентной среды
'''
